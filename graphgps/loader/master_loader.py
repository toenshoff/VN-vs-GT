...
from graphgps.loader.dataset.cor44_syntetic import Cor44_Syntetic
from graphgps.loader.dataset.cor49_syntetic import Cor49_Syntetic
    ...
    if format.startswith('PyG-'):
        ...
        if pyg_dataset_id == 'Actor':
            ...
            dataset = preformat_COCOSuperpixels(dataset_dir, name,
                                                cfg.dataset.slic_compactness)
        elif pyg_dataset_id[:5] == 'Cor44':
            dataset = preformat_Cor44_Syntetic(dataset_dir)
        elif pyg_dataset_id[:5] == 'Cor49':
            dataset = preformat_Cor49_Syntetic(dataset_dir)
        else:
...
def preformat_Cor49_Syntetic(dataset_dir):
    dataset = join_dataset_splits(
        [Cor49_Syntetic(root=dataset_dir, split=split)
         for split in ['train', 'val', 'test']]
    )
    return dataset

def preformat_Cor44_Syntetic(dataset_dir):
    dataset = join_dataset_splits(
        [Cor44_Syntetic(root=dataset_dir, split=split)
         for split in ['train', 'val', 'test']]
    )
    return dataset
